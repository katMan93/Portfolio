...
#from convGUI import main



path = 'T:/DELTA Center/Travis/Fall 2016 Data/100% Online/College'
...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    <head>
        ...
        <title>SHSU Online Enrollment - Semester Year</title>
...
"""
            ...
            wrapper= """
                        ...
                        <div id="tabs-2">
                            <p class="enrollmentText">Undergraduate Enrollment: %d</p>
                            <!-- First column of data -->
                            <span class="dataLeft">
                                <ul>
"""
            part3 = wrapper %(undergradEnroll)
            f.write(part3)
            for col in range(2, sheet.nrows):
                uNames = sheet.cell(col,3)
                uNums = sheet.cell(col,4)
                if uNames.value != xlrd.empty_cell.value:
                    if uNums.value != xlrd.empty_cell.value:
                        f.write('\t\t\t\t\t\t\t\t\t' +'<li><strong>' + uNames.value + '</strong> ' + repr(uNums.value)[:-2] + '</li>' + "\n")
            wrapper = """    
                                </ul>
                            </span>
                            <!-- Second column of data (only applicable if more than 25 entries -->
